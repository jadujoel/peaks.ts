...
declare module "peaks.js" {
	...
	type AudioOptions = OneOf<
		RemoteWaveformDataOptions,
		LocalWaveformDataOptions,
		WebAudioOptions
	>;
	...
	interface EventEmitterForPlayerEvents {
		emit<E extends keyof PlayerEvents>(
			event: E,
			...eventData: EventData<PlayerEvents[E]>
		): void;
	}

	type EventData<T> = [T] extends [(...eventData: infer U) => any]
		? U
		: [T] extends [void]
			? []
			: [T];
	...
	interface WaveformZoomView extends WaveformView {
		enableAutoScroll: (
			enable: boolean,
			options?: EnableAutoScrollOptions,
		) => void;
		scrollWaveform: (
			options: XOR<{ seconds: number }, { pixels: number }>,
		) => void;
		setStartTime: (time: number) => void;
		setWheelMode: (
			mode: "scroll" | "none",
			options?: SetWheelModeOptions,
		) => void;
		setZoom: (
			options: XOR<{ scale: number | "auto" }, { seconds: number | "auto" }>,
		) => void;
	...
	export interface PeaksInstance {
		...
		/** Events */
		on: <E extends keyof PeaksEvents>(
			event: E,
			listener: PeaksEvents[E],
		) => void;
		once: <E extends keyof PeaksEvents>(
			event: E,
			listener: PeaksEvents[E],
		) => void;
		off: <E extends keyof PeaksEvents>(
			event: E,
			listener: PeaksEvents[E],
		) => void;
	}
