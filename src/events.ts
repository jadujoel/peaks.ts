...
/**
 * Dispatches a `points.<name>` click-style event in a strictly typed way,
 * narrowing the dynamic name at the boundary.
 */
export function dispatchPointEvent(
	events: PeaksEvents,
	name: PointerInteractionName,
	payload: PointClickEvent,
): void {
	switch (name) {
		case "click":
			events.dispatch("points.click", payload);
			return;
		case "dblclick":
			events.dispatch("points.dblclick", payload);
			return;
		case "contextmenu":
			events.dispatch("points.contextmenu", payload);
			return;
	}
}

/**
 * Dispatches a `segments.<name>` click-style event in a strictly typed way,
 * narrowing the dynamic name at the boundary.
 */
export function dispatchSegmentEvent(
	events: PeaksEvents,
	name: PointerInteractionName,
	payload: SegmentClickPayload,
): void {
	switch (name) {
		case "click":
			events.dispatch("segments.click", payload);
			return;
		case "dblclick":
			events.dispatch("segments.dblclick", payload);
			return;
		case "contextmenu":
			events.dispatch("segments.contextmenu", payload);
			return;
	}
}

/**
 * Dispatches a `<viewName>.<name>` click-style event in a strictly typed
 * way, narrowing both the view name and the click name at the boundary.
 */
export function dispatchViewEvent(
	events: PeaksEvents,
	viewName: ViewName,
	name: PointerInteractionName,
	payload: ViewClickEvent,
): void {
	if (viewName === "zoomview") {
		switch (name) {
			case "click":
				events.dispatch("zoomview.click", payload);
				return;
			case "dblclick":
				events.dispatch("zoomview.dblclick", payload);
				return;
			case "contextmenu":
				events.dispatch("zoomview.contextmenu", payload);
				return;
		}
	} else {
		switch (name) {
			case "click":
				events.dispatch("overview.click", payload);
				return;
			case "dblclick":
				events.dispatch("overview.dblclick", payload);
				return;
			case "contextmenu":
				events.dispatch("overview.contextmenu", payload);
				return;
		}
	}
}
